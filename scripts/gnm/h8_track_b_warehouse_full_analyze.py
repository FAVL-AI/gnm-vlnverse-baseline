...
OUT = REPO / "assets/experiments/hospital_h8_track_b_warehouse_full_scan"
...
VISUAL_OVERRIDES = {
    "warehouse_full:p13": ("RENDER_VALID_BUT_VISUALLY_WEAK",
        "VISUAL VERIFICATION (gate 13): the contact sheet shows a STRAIGHT AISLE — goalA(S) and "
        "goalB(N) are opposite ends of the SAME aisle (collinear, sep 180 deg), distinguishable only "
        "by a pallet/obstacle at one end; the decision(E) view is a rack face of boxes, not a "
        "navigable cross-aisle. Not a genuine angular fork; DINO 0.584 is a marginal artifact. "
        "Downgraded from RENDER_VALID_JUNCTION."),
}
DIRS = ["E", "N", "W", "S"]
...
for sc in man["scenes"]:
    ...
    for pr in sc["probes"]:
        ...
        rows.append(row)
        if cls in ("RENDER_VALID_JUNCTION", "RENDER_VALID_BUT_VISUALLY_WEAK", "OPEN_FLOOR_NOT_JUNCTION"):
            sjunc.append(row)
...
with open(OUT / "warehouse_full_visual_similarity_matrix.csv", "w", newline="") as f:
...
(OUT / "warehouse_full_visual_similarity_matrix.md").write_text("\n".join(mm) + "\n")
...
for r in rows:
    if r["classification"] not in ("RENDER_VALID_JUNCTION", "RENDER_VALID_BUT_VISUALLY_WEAK", "OPEN_FLOOR_NOT_JUNCTION"):
        continue
...
(OUT / "warehouse_full_render_scan_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
...
with open(OUT / "warehouse_full_render_scan_table.csv", "w", newline="") as f:
...
rep = ["# H8-M Track-B Bounded Junction Render Scan (VALIDATION ONLY)", "",
       ...
       *[f"| {r['scene_id']} | {r['probe_id']} | {r['classification']} | {r.get('n_open')} | "
       ...
       ("- At least one RENDER_VALID_JUNCTION exists (visually verified) → **hold for review** "
        ...
        "- **No render-valid junction** in `warehouse_full`. → recommend **selecting or acquiring a "
        "new corridor-structured scene** (with real aisles / corridor forks) rather than scanning "
        "more random assets. Do NOT force a fake angular-branch claim."),
       "- Bounded scan: absence is strong evidence, not proof; cardinal sampling resolves orthogonal "
       "junctions, not oblique forks.",
       f"- **Final counts (warehouse_full): OPEN_FLOOR_NOT_JUNCTION="
       f"{cls_counts.get('OPEN_FLOOR_NOT_JUNCTION', 0)}, RENDER_VALID_BUT_VISUALLY_WEAK="
       f"{cls_counts.get('RENDER_VALID_BUT_VISUALLY_WEAK', 0)}, WALL_ONLY="
       f"{cls_counts.get('WALL_ONLY', 0)}, RENDER_VALID_JUNCTION="
       f"{cls_counts.get('RENDER_VALID_JUNCTION', 0)}.** The visually-weak cases are straight / "
       "collinear aisles (opposite-end goals along one aisle), not true angular forks.",
       "- **Track-B status: `office_isaac`, `warehouse_simple`, and `warehouse_full` have ALL failed "
       "the Track-B render-valid-junction gate.** No angular branch-choice training should begin from "
       "these scenes.",
       "- **Next step: source or build a genuinely corridor-forked T/Y/cross-junction scene** — two "
       "navigable branches diverging >= 30 deg with visually distinct, branch-specific goal views — "
       "not more Isaac stock warehouses/offices.", "",
       "## Visual verification (mandatory) & scene character",
       f"- **Class distribution (warehouse_full):** {cls_counts_str}.",
       f"- **Open-floor guard (explicit `OPEN_FLOOR_NOT_JUNCTION` class):** depth-openness in >=3 "
       "cardinal directions does NOT by itself mean a corridor junction — an OPEN HALL is open in "
       f"every direction too. Points with all 4 cardinal depths >= {OPEN_FLOOR_MIN_DEPTH} m (no "
       "bounding walls) are classified `OPEN_FLOOR_NOT_JUNCTION`. Rejected here: "
       + (", ".join(open_floor_rej) if open_floor_rej else "NONE") + ".",
       "- **Contact-sheet visual verification is MANDATORY (gate 13) before any probe is confirmed "
       "`RENDER_VALID_JUNCTION`.** Depth + embedding is necessary but NOT sufficient: in the prior "
       "office/warehouse scan an open-floor corner scored just under the DINO threshold and was only "
       "caught by visually inspecting the contact sheet. Any RENDER_VALID_JUNCTION below has been "
       "visually inspected; candidates that the images show are open floor / a shelf / a straight "
       "corridor are downgraded, not reported as forks.",
       "- **Visual-verification downgrades applied (gate 13):** "
       + ("; ".join(visual_downgrades) if visual_downgrades else "NONE")
       + (". The automated gate flagged `warehouse_full:p13` as RENDER_VALID_JUNCTION, but the "
          "contact sheet shows a straight aisle (goalA/goalB are opposite ends of one aisle, sep "
          "180 deg; the decision view is a rack face) — a distance/obstacle difference, not an "
          "angular fork — so it was downgraded. warehouse_full's scanned footprint is open floor "
          "(centre) plus straight parallel aisles along the racks: no cross-aisle / T-junction."
          if visual_downgrades else ""), "",
       "## Claim boundary",
       ...
       (f"- **warehouse_full has at least one visually-verified RENDER_VALID_JUNCTION** "
        f"({', '.join(f'{s}:{p}' for s, p in valid)}) → held for review before drive validation."
        if valid else
        "- **warehouse_full produced zero render-valid junctions — it FAILS the Track-B render-valid "
        "junction gate.**"),
       ...
       "## Artifacts",
       "`assets/experiments/hospital_h8_track_b_warehouse_full_scan/`: `warehouse_full_render_scan_manifest.json`, "
       "`warehouse_full_render_scan_table.csv`, `warehouse_full_render_scan_report.md`, "
       "`warehouse_full_visual_similarity_matrix.{csv,md}`, `contact_sheets/`, `render_metadata/`. Harness: "
       "`scripts/gnm/h8_track_b_warehouse_full_render.py` (isaac), "
       "`scripts/gnm/h8_track_b_warehouse_full_analyze.py` (this). Raw .npy in scratchpad (not committed)."]
(OUT / "warehouse_full_render_scan_report.md").write_text("\n".join(rep) + "\n")
(RM / "warehouse_full_render_scan_render_manifest.json").write_text(json.dumps(man, indent=2) + "\n")
