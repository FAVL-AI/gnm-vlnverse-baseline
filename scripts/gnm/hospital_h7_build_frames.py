"""Export H7 front-camera contact sheets from the ACTUAL recorded /camera/image_raw
stream in each episode's rosbag (NOT re-renders). start / current / goal frames +
a labelled contact sheet per episode. system python3 (rosbags + numpy + PIL).
"""
import sys, json
...
OUT = REPO / "assets/experiments/hospital_h7_collection/exports"
...
def label(img_arr, ep, split, route_type, frame, eid):
    ...
    d.text((10, H + 52), f"real Isaac hospital.usd · scene-gate PASS · {eid}",
           fill=(150, 200, 160), font=fr(13))
    d.text((10, H + 72), "H7 hospital recorded frame — actual front-RGB camera stream "
...
def contact_sheet(frames, ep, split, route_type):
    ...
    d.text((pad, 34), "RECORDED front-RGB /camera/image_raw — start → current → goal "
           "(real Isaac hospital.usd, scene-gate PASS)", fill=(30, 90, 40), font=fr(14))
    ...
    fy = hdr + pad + TH + cap + 4
    d.text((pad, fy), "H7 hospital recorded frames — actual camera stream, not a re-render. "
...
(OUT / "h7_contact_sheets_index.json").write_text(json.dumps(index, indent=2))
